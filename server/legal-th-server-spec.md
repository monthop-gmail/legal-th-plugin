...
| Encryption | TLS 1.3+ |
| Base URL (dev) | `https://svc.dev.legal-th.example.com/mcp` |
| Base URL (test) | `https://svc.test.legal-th.example.com/mcp` |
| Base URL (prd) | `https://svc.legal-th.example.com/mcp` |
...
- ใช้ CF Tunnel แทน port forwarding (ปลอดภัยกว่า expose port ตรง)
- Tunnel service อยู่ใน compose base (`--no-autoupdate`)
- สร้าง Tunnel + Public Hostname บน Cloudflare Dashboard
- แต่ละ env ใช้ Tunnel คนละตัว (token แยก)

#### Secret Management (SOPS + age)

- `.env` → gitignored (ห้าม commit)
- `.env.enc` → encrypted ด้วย SOPS + age (commit ได้)
- `.sops.yaml` → กำหนด public keys ที่มีสิทธิ์ decrypt (commit ได้)
- Key distribution ผ่าน 1Password / Bitwarden / Signal เท่านั้น
...
| Environment | Branch | DNS Pattern |
|-------------|--------|-------------|
| dev | `feature/*` | `svc.dev.legal-th.example.com` |
| test | `develop` | `svc.test.legal-th.example.com` |
| prd | `main` | `svc.legal-th.example.com` |

**Phase 1 (MVP):**
```
Docker Compose on VPS / DigitalOcean Droplet
├── Single server instance
├── PostgreSQL container
├── Cloudflare Tunnel (HTTPS)
├── SOPS + age secrets
└── GitHub Actions CI/CD
ค่าใช้จ่ายประมาณ: $10-30/เดือน
```
...
├── Elasticsearch container (ค้นหาดีขึ้น)
├── Modular compose (include:)
└── Replicas (app x2)
ค่าใช้จ่ายประมาณ: $50-150/เดือน
```
...
| # | คำถาม | สถานะ | ตัดสินใจแล้ว |
|---|-------|-------|------------|
| 1 | **Tech stack** | ✅ ตัดสินใจแล้ว | Node.js (TypeScript) + Fastify + Drizzle + PostgreSQL |
| 2 | **Cloud platform** | ✅ ตัดสินใจแล้ว | Docker Compose on VPS + Cloudflare Tunnel |
| 3 | **Infrastructure pattern** | ✅ ตัดสินใจแล้ว | ตาม dev-env-skill checklist (SOPS + CF Tunnel + GitHub Actions) |
| 4 | **License model** | ✅ ตัดสินใจแล้ว | MIT (Open source) |
| 5 | **ข้อมูลกฎหมาย** | ❓ รอตัดสินใจ | ใครเป็นผู้รวบรวมและตรวจสอบ? |
| 6 | **ลำดับความสำคัญ** | ❓ รอตัดสินใจ | กฎหมาย 50 ฉบับแรกเอาอะไรบ้าง? |
| 7 | **Domain name** | ❓ รอตัดสินใจ | `svc.legal-th.example.com` หรืออื่น? |
